    ...
    case @@mstep
      ...
      when MENU_STEP_FIND_CANDI
        ...
        if user_msg == "이전"
          ...
          msg = @juso_menu[@@fstep - 10]
          basic_keyboard = @juso_menu.keys[@@fstep - 10]
        elsif user_msg == "다시"
        ...
        if @@fstep == FUNC_STEP_TYPE_OF_ELECTION
          $_sgcode = @sun_code[user_msg]  # 선거 코드 물어본거 여기 저장.
          @@fstep = FUNC_STEP_CHOICE_SIDO # 다음 단계로 이동.
          
          msg = @juso_menu.keys[@@fstep - 10]
          basic_keyboard = @juso_menu[@@fstep - 10]
          @@rem.push(@@fstep)
          
        elsif @@fstep == FUNC_STEP_CHOICE_SIDO
          
          q1 = "\"wiwname\"::text LIKE \"%#{user_msg[0,2]}%\""
          res = Sido.where(q1)#.pluck(:wiwid)

          if res.length == 0
            q2 = "\"findlist\" LIKE \"%#{user_msg[0,2]}%\""
            res = Sido.where(q2).pluck(:wiwid)
          end
          
          if res.length == 0
            @@fstep = FUNC_STEP_CHOICE_SIDO          
          else
            fin = Sido.find_by("wiwid": res[0])                # 어떤 광역시 인지 도인지 확인.
        ...
        elsif @@fstep == FUNC_STEP_CHOICE_SIGUN
        ...
        elsif @@fstep == FUNC_STEP_CHOICE_GU
        ...
        elsif @@fstep == FUNC_STEP_CHOICE_EMD
        ...
        if user_msg == "시도"
          ...
          gusigun.each do |gsg|
            ...
            js["emdMap"].each do |emd|
              Emd.create(gusigun_id: gsg.id, towncode: gsg.towncode, emdcode: emd["CODE"], emdname: emd["NAME"], findlist: juso[emd["NAME"]])
              
              if juso[emd["NAME"]].nil?
